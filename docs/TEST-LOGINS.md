...
| Outbound SMS redirect (pilot) | Set `TWILIO_SMS_REDIRECT_TO=+16046792422` in `.env.local` / staging / beta. On **production** also set `TWILIO_ALLOW_SMS_REDIRECT=true` (redirect is ignored in production without that flag). |
