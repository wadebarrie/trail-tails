...
| Route order | Admin drag-and-drop; default on `dogs.route_sort_order`, per-day override on `stops.sort_order` |
...
**Route order:** Pickup and dropoff stops for the same dog share the same `sort_order`. Driver UI lists stops `ORDER BY sort_order`. Admin reorders via drag-and-drop on the default route (updates `dogs.route_sort_order`) or on a specific day (updates `stops.sort_order` for that hike).
...
FOR date = tomorrow IN company timezone:
  ...
  FOR each active dog:
    IF day_of_week IN dog_schedule_days
      ...
      INSERT stop (dropoff) — same window + sort_order snapshot
```
